...
export function Places ({quant}) {
    return (
        ...
        <section className={`${styles.placesBlock}`}>
            <svg width="200px" height="200px" viewBox="0 0 512 512" version="1.1">
            ...
            <span>{quant}</span>
            <p className={`${styles.placesText}`}>{(quant==1)?'LUGAR' : 'LUGARES'}<br/>{(quant==1)?'RESERVADO' : 'RESERVADOS'} PARA TI</p>
        </section>
        </>
    )
}